...
CollectionResult TryCollectPoint(Point a, Point b, Point c) {
    ...
    // поскольку при сборе лута придётся учитывать перемещение даже на небольшое расстояние.

    if (b.x == a.x && b.y == a.y) {
        //Нулевые векторы должны отбрасываться до вызова функции
        throw std::invalid_argument("Zero movement vector");
    }
