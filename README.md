...
В рамках разработки проекта реализован Backend, Frontend был предоставлен командой [Яндекс Практикум](https://practicum.yandex.ru/).

В проекте введено разделение для доступа к статическим файлам (дороги, здания, офисы и прочие модели) и доступа к API.
...
Доступ к API организован с помощью boost::asio::strand, для сохранения консистентного состояния игры. На данный момент реализован 1 strand на всю игру, значит один стренд на все игровые сессии.

Для чтения параметров командной строки использован boost::program_options. Поддерживаются следующие команды:
```bash
Allowed options:
...
- **Conan** - требуется обязательно версия 1.x. Для работы с 2.x потребуется изменить `conanfile`.
...
```
### Локальный запуск:
Работоспособность приложения проверялось только на Ubuntu, с запуском на Windows могут быть проблемы. Для запуска на Windows рекомендую воспользоваться Docker.
...
После сборки и запуска, откройте в браузере адрес `http://127.0.0.1:8080/` и наслаждайтесь игрой.
